  ...
  class ChatUI {
    ...
    _renderShell() {
        ...
        <footer class="chat-app-compose-wrap">
          ...
          <form class="chat-app-compose" id="chat-app-form">
            <div class="compose-inner">
              <textarea id="chat-app-input" rows="1" placeholder="Messaggio per Abra…" autocomplete="off"></textarea>
            ...
            <p class="compose-hint">Invio per inviare · Shift+Invio nuova riga</p>
          </form>
        </footer>`;
      ...
      if (this.opts.suggestions.length) {
        ...
        if (this.opts.variant === 'thread') {
          chips.classList.add('chat-app-chips-thread');
          this.container.querySelector('.chat-app-compose-wrap')?.insertBefore(
            chips,
            this.container.querySelector('#chat-app-form')
          );
          if (chipsLabel) chipsLabel.hidden = false;
        }
      } else {
        chips.remove();
        chipsLabel?.remove();
